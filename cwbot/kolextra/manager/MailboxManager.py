...
from cwbot.kolextra.request.GetMessagesRequest import GetMessagesRequest
...
class MailboxManager(object):
    """
    This class manages a user's mailbox. It can be used as a wrapper to 
    ...
    def setOldestFirst(self, oldestFirst):
        """Sets whether the user wants their messages sorted oldest first or 
        newest first."""
        with self.__lock:
    ...
    def getAllMessages(self, box="Inbox", openGiftPackages=False, 
        ...
        packages can be removed from the item array in each message.
        """
        with self.__lock:
            # To speed up the request, make sure we are requesting 100
            # messages per page.
            ...
            if openGiftPackages:
                ...
                for m in messages:
                    if len(m["items"]) == 1 and m["meat"] == 0:
                        if ("type" in m["items"][0] and 
                                m["items"][0]["type"] == "gift package"):
                            messagesWithUnopenedPackages.append(m["id"])
    
                # If there are gift packages, open them and then re-fetch
                # the messages.
                if len(messagesWithUnopenedPackages) > 0:
                    ...
                    # known bug in KoL. Apparently it is somehow possible
                    # to send a gift package with nothing inside of it. Just
                    # delete the package from the message and go on.
                    # If a new message has arrived with a gift package, ignore
                    # it for now. We can look at it next time.
                    messagesToIgnore = []
                    for m in messages:
                        if len(m["items"]) == 1 and m["meat"] == 0:
                            if ("type" in m["items"][0] and
