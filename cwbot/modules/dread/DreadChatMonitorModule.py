...
class DreadChatMonitorModule(BaseDungeonModule):
    """
    ...
    def __init__(self, manager, identity, config):
        self._dreadAdventureCounts = collections.defaultdict(lambda: 0)
        ...
        super(DreadChatMonitorModule, self).__init__(manager, identity, config)
    ...
    def _configure(self, config):
        try:
            ...
            raise Exception(
                    "Error in module config: (DreadChatMonitorModule) "
    ...
    def initialize(self, state, initData):
        ...
        self._dreadAdventureCounts.clear()

        for dreadevent in (item for item in self._lastEvents
                               if item['category'] in self.zones):
            uname = dreadevent['userName']
            self._dreadAdventureCounts[uname] += dreadevent['turns']
    ...
    def dispatchKmail(self, uid, dailyDigest=False, noThrow=False):
        with self._lock:
            ...
            txt = ("Dread chat monitor violations for {} "
            ...
            if len(violators) > 0:
                ...
                for uname in violators:
                    ...
                    # mark players in clan chat (but not in dread)
                    if any(v.violation and v.data.get('inClan', False)
                           for v in vList):
                        s = "*" + s

                    # note if they joined /dread
                ...
                txt += "No users were in violation of Dread chat rules."
            k = Kmail(uid=uid, text=txt)
            self.log("Sending DreadChatMon kmail to {}...".format(uid))
            self.sendKmail(k)
            return True
    ...
    def registerClear(self, userName):
        with self._lock:
            ...
            self._violations[userName] = violations
        self.debugLog("Detected {} adventuring in dread while in channel."
                      .format(userName))
    ...
    def _availableCommands(self):
        return {'chatmonitor': "Display players who have violated Dread "
    ...
    def _getAdventureCounts(self, events):
        ...
        for dreadevent in (item for item in events
                               if item['category'] in self.zones):
            uname = dreadevent['userName']
            advCounts[uname] += dreadevent['turns']
        return advCounts
    ...
    def _getUsersToCheck(self, events, adventureCounts):
        ...
        for uname,turns in list(adventureCounts.items()):
            if turns > self._dreadAdventureCounts.get(uname, 0):
    ...
    def _heartbeat(self):
        ...
        with self._lock:
            ...
            self._dreadAdventureCounts = advCounts
